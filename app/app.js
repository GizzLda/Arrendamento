...
const buildSummaryRow = (label, values, rowClass = "", isTotal = false) => {
  const total = values.reduce((sum, value) => sum + value, 0);
  const totalClass = isTotal ? "summary-total" : "";
  return `
    <tr class="${rowClass} ${totalClass}">
      <th>${label}</th>
    ...
    <table class="summary-table">
      ...
      <tbody>
        ${incomeData
          ...
          .join("")}
        ${buildSummaryRow("Total Receitas", incomeTotals, "income-total", true)}
      </tbody>
      <thead>
      ...
      <tbody>
        ${expenseData
          ...
          .join("")}
        ${buildSummaryRow("Total Despesas", expenseTotals, "expense-total", true)}
      </tbody>
      <tfoot>
        <tr class="summary-section result-row summary-total">
          <th>Resultado</th>
          ${resultTotals
            .map(
              (value) =>
